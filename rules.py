...
def total_revenue(data: dict, financial_index):
    ...
    if 0 <= financial_index < len(financials):
        pnl_section = financials[financial_index].get("pnl", {})
        net_revenue = pnl_section.get("lineItems", {}).get("netRevenue", 0.0)
        return net_revenue
    return 0.0
...
def total_borrowing(data: dict, financial_index):
    ...
    if 0 <= financial_index < len(financials):
        balance_sheet = financials[financial_index].get("bs", {})
        
        # Extract long-term and short-term borrowings
        long_term_borrowings = balance_sheet.get("lineItems", {}).get("longTermBorrowings", 0.0)
        short_term_borrowings = balance_sheet.get("lineItems", {}).get("shortTermBorrowings", 0.0)
        
        # Calculate the total borrowings
        total_borrowings = long_term_borrowings + short_term_borrowings
        
        # Get the total revenue using the total_revenue function
...
def iscr(data: dict, financial_index):
    ...
    if 0 <= financial_index < len(financials):
        ...
        # Extract relevant values from Profit and Loss section
        pbitda = pnl_section.get("lineItems", {}).get("pbitda", 0.0)
        depreciation = pnl_section.get("lineItems", {}).get("depreciation", 0.0)
        interest_expenses = pnl_section.get("lineItems", {}).get("interestExpenses", 0.0)
        
        # Calculate ISCR (Interest Service Coverage Ratio)
        iscr_value = (pbitda + 1) / (interest_expenses + 1) if (interest_expenses + 1) != 0 else 0.0
